...
export default async function anthropicClient(prompt: string): Promise<ContentBlock[]> {
    ...
    const promptSended = 'Please answer with 50 characters max.' +
        `${prompt}`
    ...
    const response = await client.messages.create({
        max_tokens: 1024,
        messages: [
            { role: 'user', content: promptSended }
        ],
